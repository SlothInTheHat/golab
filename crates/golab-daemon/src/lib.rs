...
use golab_core::{workspace, Store};
...
enum WsFrame {
    ...
    Event { event: golab_core::model::Event },
...
#[derive(serde::Serialize)]
struct Snapshot {
    status: golab_core::work::StatusSummary,
    sessions: Vec<golab_core::session::SessionView>,
    activity: Vec<golab_core::activity::ActivityView>,
    notifications: Vec<golab_core::notice::Notice>,
    plan: Option<Box<golab_core::schedule::Plan>>,
    arch: Option<Box<golab_core::arch::ArchGraph>>,
    goals: Vec<golab_core::context::GoalWithProgress>,
    at: i64,
}
...
pub async fn serve(root: PathBuf, addr: &str, watch: bool) -> Result<()> {
    let store = Store::open(&golab_core::db_path(&root))?;
    ...
    if watch {
        ...
        std::thread::spawn(move || {
            if let Err(e) = watcher::watch(&watch_root, watcher::Stop::new(), |_, _| {}) {
                eprintln!("golab: watcher stopped: {e:#}");
            }
        });
    }
...
async fn api_symbols(State(state): State<AppState>, Query(q): Query<SymbolQuery>) -> Response {
    ...
    let kind = q
        .kind
        .as_deref()
        .and_then(golab_core::model::SymbolKind::parse);
...
fn default_ttl() -> i64 {
    golab_core::lease::DEFAULT_TTL_SECS
}
...
fn default_kind() -> String {
    golab_core::model::request_kind::QUESTION.to_string()
}
...
async fn api_open_request(
    ...
    if body.kind == golab_core::model::request_kind::LEASE_TRANSFER {
...
async fn api_endpoints(State(state): State<AppState>) -> Response {
    let store = state.store.lock().expect("store poisoned");
    match store.symbols_with_role(golab_core::model::Role::Api) {
        Ok(v) => Json(v).into_response(),
        Err(e) => fail(e),
    }
}
...
async fn api_tables(State(state): State<AppState>) -> Response {
    let store = state.store.lock().expect("store poisoned");
    let result = store
        .list_symbols(None, Some(golab_core::model::SymbolKind::Table), None, 500)
        .and_then(|tables| {
            let mut out = Vec::new();
            for t in tables {
...
async fn api_services(State(state): State<AppState>) -> Response {
    let store = state.store.lock().expect("store poisoned");
    let result = store
        .list_symbols(None, Some(golab_core::model::SymbolKind::Service), None, 200)
        .and_then(|services| {
            let mut out = Vec::new();
            for s in services {
                ...
                let depends_on: Vec<String> = n
                    .callees
                    .iter()
                    .filter(|x| x.kind == golab_core::model::EdgeKind::Imports)
...
async fn api_owners(State(state): State<AppState>, Query(q): Query<OwnersQuery>) -> Response {
    let owners = golab_core::topology::Owners::load(&state.root);
    match q.path {
        Some(path) => Json(json!({
            "path": path,
...
async fn api_task_state(
    ...
    let Some(target) = golab_core::model::TaskState::parse(&body.state) else {
...
async fn api_capability(
    ...
    let capability = match body.capability.as_deref() {
        None | Some("") => None,
        Some(c) => match golab_core::model::Capability::parse(c) {
            Some(c) => Some(c),
