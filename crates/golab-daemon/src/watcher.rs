...
//! thread beside the HTTP server, or on its own via `golab index --watch`,
...
use anyhow::Result;
use golab_core::notify::scan_and_notify;
use golab_core::scan::{self, ScanStats};
...
pub fn watch(
    ...
    loop {
        ...
        // The watcher follows the default repo only — see `golab repo`.
        match scan_and_notify(&mut store, golab_core::ids::DEFAULT_REPO_ID, &root, targets, false) {
            Ok(stats) => on_index(&paths, &stats),
            Err(e) => eprintln!("golab: re-index failed: {e:#}"),
        }
    }
}
...
mod tests {
    ...
    fn a_saved_file_is_reindexed_without_a_manual_scan() {
        ...
        scan::scan(&mut store, golab_core::ids::DEFAULT_REPO_ID, &root, &[], false).unwrap();
        ...
        let store = Store::open(&golab_core::db_path(&root)).unwrap();
        assert!(
