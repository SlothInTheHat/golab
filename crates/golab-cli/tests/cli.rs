//! End-to-end tests driving the real `golab` binary.
//!
//! The lease tests in golab-core prove the logic; these prove the property
...
/// The hook must not depend on `golab` being on PATH: git runs hooks with a
...
fn the_installed_hook_points_at_a_real_binary() {
    ...
    assert!(script.contains("GOLAB_SKIP"), "there should be an escape hatch");
...
fn commands_outside_a_workspace_explain_themselves() {
    ...
    assert!(String::from_utf8_lossy(&out.stderr).contains("golab init"));
}
