...
use golab_core::activity::{self, ActivityView};
use golab_core::arch::{ArchGraph, ArchKind, ArchNode, NodeDetail};
use golab_core::context::{AgentContext, TaskContext};
use golab_core::guard::{GuardReport, GuardVerdict};
use golab_core::model::*;
use golab_core::session::SessionView;
use golab_core::work::{AgentView, StatusSummary, TaskView};
...
pub fn scan_line(stats: &golab_core::scan::ScanStats, st: &Style) -> String {
...
pub fn goal_progress_block(g: &Goal, progress: &GoalProgress, tasks: &[TaskView], st: &Style) -> String {
    ...
    if tasks.is_empty() {
        out.push_str(&format!("  {}\n", st.dim("no tasks yet — try `golab goal decompose` or `golab goal suggest`")));
    } else {
        for t in tasks {
            out.push_str(&format!("  {}\n", task_line(t, st)));
        }
...
pub fn suggestion_block(goal_id: &str, suggestions: &[TaskSuggestion], st: &Style) -> String {
    ...
    for s in suggestions {
        ...
        out.push_str(&format!(
            "    {}\n",
            st.dim(&format!(
                "golab goal decompose {goal_id} --task \"{}\" {}",
                s.title,
                s.symbols
                    .iter()
...
pub fn request_line(r: &Request, st: &Style) -> String {
    ...
    let deadline = match r.seconds_until_deadline(golab_core::now_ms()) {
...
pub fn scheduled_line(t: &golab_core::schedule::ScheduledTask, st: &Style) -> String {
...
pub fn observe_block(
    ...
    let contended: Vec<&golab_core::schedule::ScheduledTask> = plan
        .waves
        .first()
...
pub fn plan_block(plan: &golab_core::schedule::Plan, st: &Style) -> String {
    let mut out = String::new();
...
pub fn throughput_block(t: &golab_core::work::Throughput, st: &Style) -> String {
...
pub fn arch_block(g: &ArchGraph, st: &Style) -> String {
    let mut out = String::new();
    if g.nodes.is_empty() {
        return format!("{}\n", st.dim("nothing indexed — run `golab index`"));
    }
...
pub fn arch_node_block(d: &NodeDetail, st: &Style) -> String {
    ...
    let names = |refs: &[golab_core::arch::ArchRef]| -> Vec<String> {
...
pub fn activity_line(a: &ActivityView, st: &Style) -> String {
    ...
    line.push_str(&st.dim(&if a.live {
        let held = (golab_core::now_ms() - a.activity.started_at) / 1000;
