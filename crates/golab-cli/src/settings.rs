...
mod tests {
    ...
    fn installing_twice_adds_one_entry() {
        let mut s = json!({});
        assert!(upsert_hook(&mut s, "PreToolUse", Some("Edit"), "/bin/golab hook guard", 10));
        assert!(!upsert_hook(&mut s, "PreToolUse", Some("Edit"), "/bin/golab hook guard", 10));
    ...
    fn an_unrelated_hook_for_another_tool_survives_both_ways() {
        ...
        upsert_hook(&mut s, "PreToolUse", Some("Edit"), "/bin/golab hook guard", 10);
    ...
    fn uninstalling_prunes_what_it_empties() {
        ...
        upsert_hook(&mut s, "SessionStart", None, "/bin/golab hook session-start", 15);
    ...
    fn uninstall_matches_even_after_the_binary_moved() {
        let mut s = json!({});
        upsert_hook(&mut s, "PreToolUse", Some("Edit"), "/old/path/golab hook guard", 10);
    ...
    fn unrelated_keys_and_nesting_round_trip() {
        ...
        edit_json_file(&path, |s| {
            upsert_hook(s, "SessionEnd", None, "/bin/golab hook session-end", 15);
            Ok(())
        })
        .unwrap();
    ...
    fn an_mcp_server_entry_replaces_only_its_own_key() {
        ...
        assert!(upsert_mcp_server(
            &mut doc,
            "golab",
            json!({ "command": "/bin/golab", "args": ["mcp"] })
        ));
        ...
        assert!(remove_mcp_server(&mut doc, "golab"));
