...
//!
//! This is the layer that turns golab from "another CLI" into infrastructure a
...
//! - **A refusal is not an error.** golab's exit-code doctrine — `0` yes, `1` a
...
//! `golab-core` contains no printing at all, which is what makes calling it
//! from here safe; all of golab's output lives in `golab-cli`.
