...
//! as `mcp__golab__next_task`), so a `golab_` prefix would stutter.
//!
...
fn next_task(s: &mut Session, args: &Value) -> Result<ToolOutcome> {
    ...
    // Mirrors `golab continue` exactly, including the order: already working
...
fn check_edit(s: &mut Session, args: &Value) -> Result<ToolOutcome> {
    ...
    let text = match report.verdict {
        golab_core::guard::GuardVerdict::Denied => format!(
            "DO NOT EDIT. {}. {}",
            report.summary,
            report
                .suggestions
