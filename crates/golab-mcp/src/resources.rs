...
pub fn read(s: &mut Session, params: &Value) -> Result<Value> {
    ...
    let body = match uri {
        "golab://status" => serde_json::to_value(s.store.status(30)?)?,
        "golab://goals" => {
            let mut out = Vec::new();
        ...
        "golab://plan" => serde_json::to_value(s.store.plan()?)?,
        "golab://inbox" => serde_json::to_value(s.store.requests(
            Some(&s.agent),
            Direction::Inbox,
            true,
        )?)?,
        "golab://context" => serde_json::to_value(s.store.agent_context(&s.agent, 2)?)?,
