...
use atlas_core::session::{transport, NewSession};
use atlas_core::workspace::{guard_workspace, locate};
...
///
/// Derived from the same inputs `atlas mcp` uses, so a hook process and the
...
pub fn guard(root: &Path, mode: &str) -> Result<ExitCode> {
    ...
    let Ok(mut store) = Store::open(&atlas_core::db_path(root)) else {
    ...
    let mut worst: Option<atlas_core::guard::GuardReport> = None;
    for path in &paths {
...
fn record_edit_window(store: &mut Store, agent: &str, report: &atlas_core::guard::GuardReport) {
...
fn deny_reason(report: &atlas_core::guard::GuardReport) -> String {
    let mut msg = format!("atlas: {}.", report.summary);
        ...
        .find(|s| s.action == "request-transfer" || s.action == "wait")
    {
        match s.action.as_str() {
            ...
            _ => msg.push_str(&format!(
                " Ask for it with the atlas `ask` tool (kind=lease-transfer, symbol=\"{}\"), \
                 or run: {}",
                s.symbol, s.command
            )),
        }
...
fn brief(agent: &str, c: &atlas_core::context::AgentContext) -> String {
    let mut out = format!(
        "This repository is coordinated by atlas; you are `{agent}`. \
         Other people and agents may be editing it right now. Call the atlas \
    ...
    if let Some(task) = &c.task {
        ...
        out.push_str(&format!(
            "{} task(s) are ready to start; call the atlas `next_task` tool to take one.\n",
            c.startable.len()
        ));
    }
...
pub fn post_tool(root: &Path) -> Result<ExitCode> {
    ...
    for path in &paths {
        ...
        let symbol_id = atlas_core::ids::file_symbol_id(&repo_id, &rel);
...
mod tests {
    ...
    fn a_denial_says_how_to_get_to_yes() {
        use atlas_core::guard::{GuardReport, GuardSuggestion};
        let report = GuardReport {
            ...
            suggestions: vec![GuardSuggestion {
                ...
                command: "atlas request lease src/pay.ts:charge".into(),
                tool: "ask".into(),
            }],
    ...
    fn an_about_to_lapse_lease_says_wait_rather_than_ask() {
        ...
        let report = GuardReport {
            ...
            suggestions: vec![GuardSuggestion {
                ...
                command: "atlas lease acquire src/pay.ts:charge --wait 9".into(),
