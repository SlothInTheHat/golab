//! `atlas` — command line for the collaborative agent runtime.
//!
...
use atlas_core::guard::GuardVerdict;
use atlas_core::lease::{AcquireOptions, DEFAULT_TTL_SECS};
use atlas_core::model::*;
use atlas_core::workspace::{check_workspace, diff_workspace, guard_workspace, scan_workspace};
use atlas_core::{find_workspace, Store};
use render::Style;
use serde_json::json;
...
#[command(
    name = "atlas",
    version,
    about = "Git tracks code. atlas tracks work: symbol leases, task graph and shared state for humans and AI agents.",
    disable_help_subcommand = true
)]
struct Cli {
    /// Workspace root (defaults to the nearest .atlas/ ancestor).
    ...
    /// Acting agent. Falls back to $ATLAS_AGENT, then .atlas/agent.
    #[arg(long, global = true)]
    agent: Option<String>,
...
enum Command {
    ...
    /// Install the git pre-commit hook that runs `atlas check`.
    #[command(subcommand)]
    Hook(HookCmd),
    /// Start the workspace: index the repository, serve the dashboard, and
    /// keep both current as files change.
    ///
    /// The one command to run in a repository you are working in. `serve` is
    /// the same server without the index-first step, for a workspace that is
    /// already built.
    Dev {
        ...
        #[arg(long)]
        no_watch: bool,
        /// Skip the initial index (it is incremental, so this is rarely worth it).
        #[arg(long)]
        no_index: bool,
    },
    ...
    /// Speak MCP on stdio so a coding tool drives atlas directly.
    ///
    /// Not run by hand: a client launches it (`atlas hook install --mcp`
    ...
    Mcp {
        /// Pin the agent name. Otherwise `$ATLAS_AGENT`, else `<tool>-<user>`.
        #[arg(long = "as")]
        as_agent: Option<String>,
        /// Tool slug, when the client's handshake does not name itself.
        #[arg(long)]
        tool: Option<String>,
        #[arg(long, default_value_t = atlas_mcp::DEFAULT_HEARTBEAT_SECS)]
        heartbeat_secs: u64,
...
enum RepoCmd {
    ...
    Add {
        /// Relative to the directory containing .atlas/ — not absolute, so
...
enum HookCmd {
    ...
    /// hook, which is all `atlas hook install` has ever done.
    Install {
        /// The git pre-commit hook that runs `atlas check`.
        #[arg(long)]
        git: bool,
        /// `.claude/settings.json` hooks: join, guard, progress, leave.
        #[arg(long)]
        claude_code: bool,
        /// `.mcp.json` entry so the editor launches `atlas mcp` itself.
        #[arg(long)]
        mcp: bool,
...
fn run(cli: &Cli, st: &Style) -> Result<ExitCode> {
    if let Command::Init { path } = &cli.command {
        ...
        if cli.json {
            ...
            println!(
                "{} runtime ready at {}\n  next: {}",
                st.green("✔"),
                st.bold(&root.join(".atlas").display().to_string()),
                st.dim("atlas scan && atlas agent register <name>")
            );
        }
    ...
    match &cli.command {
        ...
        Command::Index { watch } => {
            ...
            // default repo for now — see `atlas repo` in the README).
            let stats = scan_workspace(&mut store, &root, &[], false)?;
            if !cli.json {
                println!("{}", render::scan_line(&stats, st));
                println!(
            ...
            let json = cli.json;
            atlas_daemon::watcher::watch(
                &root,
                atlas_daemon::watcher::Stop::new(),
                |paths, stats| {
                    if json {
                        ...
                        println!(
                            "{} {} {}",
                            st.dim(&render::timestamp(atlas_core::now_ms())),
                            st.green("↻"),
        ...
        Command::Owners { path } => {
            let owners = atlas_core::topology::Owners::load(&root);
                ...
                for node in &impact {
                    let owner = match &node.lease {
                        Some(l) => st.red(&format!(
                            "  ← leased by {} for {}",
                            l.agent,
                            render::duration(l.seconds_left(atlas_core::now_ms()))
                        )),
                        None => String::new(),
                    };
                    println!(
        ...
        Command::Watch { since, once, tail } => {
            ...
            // `atlas --json watch` emitted a header of prose and then started
            // streaming JSON.
            let show = |e: &atlas_core::model::Event| -> Result<()> {
                if cli.json {
        ...
        Command::Dev {
            ...
            // Index before serving rather than after: the dashboard's first
            // frame carries the architecture graph, and on a cold workspace
            // that frame would otherwise be empty for as long as the initial
            // scan takes. The scan is incremental, so on a warm one this is
            // near-free.
            if !*no_index {
                let stats = scan_workspace(&mut store, &root, &[], false)?;
                if !cli.json {
                    println!(
                        ...
                        st.green("✔"),
                        st.dim(&format!(
                            "{} file(s), {} symbol(s), {} edge(s)",
                            stats.files_indexed, stats.symbols, stats.edges
                        ))
                    );
                }
            }
            ...
            if !cli.json {
                ...
                println!(
                    "{}",
                    st.dim(&match live {
                        0 => "no workers connected yet — open an editor, or run `atlas swarm join <name>`"
                            .to_string(),
            ...
            let rt = tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()?;
            rt.block_on(atlas_daemon::serve(root.clone(), &addr, !*no_watch))?;
        }
        ...
        Command::Mcp {
            ...
            let outcome = atlas_mcp::serve(atlas_mcp::McpConfig {
                agent: as_agent.clone().or_else(|| cli.agent.clone()),
                tool: tool.clone(),
                heartbeat_secs: *heartbeat_secs,
                keep_leases: *keep_leases,
                ..atlas_mcp::McpConfig::new(root.clone())
            });
            if let Err(e) = outcome {
                eprintln!("atlas mcp: {e:#}");
...
fn print_neighbors(label: &str, items: &[atlas_core::graph::Neighbor], st: &Style) {
    if items.is_empty() {
        return;
    }
...
fn agent_cmd(
    ...
    match cmd {
        AgentCmd::Register { name, kind, capability } => {
            ...
            std::fs::write(root.join(atlas_core::RUNTIME_DIR).join("agent"), name).ok();
            if cli.json {
...
fn report_task(cli: &Cli, st: &Style, t: &atlas_core::work::TaskView) {
    if cli.json {
...
fn request_cmd(
    ...
    use atlas_core::model::request_kind;
...
/// `atlas` is usually not on PATH, so a config that just says `atlas` fails
...
fn hook_cmd(cli: &Cli, st: &Style, root: &Path, cmd: &HookCmd) -> Result<ExitCode> {
    match cmd {
        HookCmd::Install {
            ...
            // Bare `atlas hook install` has always meant the git hook, and
            // still does.
            ...
            if *mcp {
                ...
                if path.exists() {
                    settings::edit_json_file(&path, |d| {
                        removed = settings::remove_mcp_server(d, "atlas");
                        Ok(())
                    })?;
                }
...
fn install_git_hook(root: &Path) -> Result<serde_json::Value> {
    ...
    let script = format!(
        "#!/bin/sh\n\
         ...
         # Set ATLAS_SKIP=1 to bypass for one commit.\n\
         [ -n \"$ATLAS_SKIP\" ] && exit 0\n\
         exec \"{exe}\" check\n"
    );
...
fn install_mcp_server(root: &Path, as_agent: Option<&str>) -> Result<serde_json::Value> {
    ...
    settings::edit_json_file(&path, |d| {
        changed = settings::upsert_mcp_server(d, "atlas", entry.clone());
        Ok(())
    })?;
...
fn workspace_root(cli: &Cli) -> Result<PathBuf> {
    ...
    find_workspace(&cwd)
        .ok_or_else(|| anyhow!("no atlas workspace here or in any parent — run `atlas init` first"))
}

fn optional_agent(cli: &Cli, root: &Path) -> Option<String> {
    cli.agent
        .clone()
        .or_else(|| std::env::var("ATLAS_AGENT").ok().filter(|s| !s.is_empty()))
        .or_else(|| {
            std::fs::read_to_string(root.join(atlas_core::RUNTIME_DIR).join("agent"))
                .ok()
...
fn require_agent(cli: &Cli, root: &Path) -> Result<String> {
    optional_agent(cli, root).ok_or_else(|| {
        anyhow!(
            "no agent identity — pass --agent, set ATLAS_AGENT, or run `atlas agent register <name>`"
        )
    })
}
