...
/// The agent name a Claude Code session derives with ATLAS_USER=tester.
...
#[test]
fn the_guard_hook_never_fails_an_edit_because_atlas_is_unhappy() {
...
fn installing_the_mcp_server_registers_it_without_a_hardcoded_root() {
    ...
    let atlas = &doc["mcpServers"]["atlas"];
    assert!(atlas["command"].as_str().unwrap().contains("atlas"));
    let args: Vec<&str> = atlas["args"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let doc: Value =
        serde_json::from_str(&std::fs::read_to_string(d.join(".mcp.json")).unwrap()).unwrap();
    assert!(doc["mcpServers"]["atlas"].is_null());
    assert_eq!(doc["mcpServers"]["other"]["command"], "other-server");
}
...
fn bare_hook_install_still_means_the_git_hook() {
    ...
    assert!(script.contains("atlas"), "{script}");
    assert!(
