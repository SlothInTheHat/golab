//! `atlas guard`, `atlas context` and `atlas session`, end to end.
//!
...
fn guard_resolves_identity_the_same_way_every_other_command_does() {
    ...
    // `swarm join` wrote `.atlas/agent`, so a bare invocation is bob (the
...
fn guard_without_any_identity_is_an_error_not_a_denial() {
    // A workspace nobody has joined: there is no `.atlas/agent` to fall back
