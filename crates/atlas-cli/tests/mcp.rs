//! `atlas mcp`, end to end, over the real stdio protocol.
//!
...
impl Mcp {
    fn spawn(dir: &Path, args: &[&str]) -> Mcp {
        ...
        let mut child = cmd.spawn().expect("failed to spawn atlas mcp");
...
fn initialize_registers_the_agent_and_opens_a_session() {
    ...
    assert_eq!(info["serverInfo"]["name"], "atlas");
    assert_eq!(
...
fn two_tools_in_one_workspace_get_distinct_identities() {
    ...
    assert!(
        !d.join(".atlas").join("agent").exists(),
...
fn stdout_carries_only_protocol_frames() {
    ...
    mcp.req("resources/read", json!({ "uri": "atlas://status" }));
    mcp.req("ping", json!({}));
...
fn the_tool_surface_is_documented_and_schema_bearing() {
    ...
    for t in tools {
        ...
        assert!(!name.starts_with("atlas_"), "clients namespace these already: {name}");
        assert!(
...
fn notices_ride_along_on_a_tool_result_the_model_was_making_anyway() {
    ...
    assert!(
        text.contains("[atlas]") && text.contains("carol asks"),
...
fn a_quiet_workspace_adds_nothing_to_a_tool_result() {
    ...
    assert!(!raw["content"][0]["text"].as_str().unwrap().contains("[atlas]"));
