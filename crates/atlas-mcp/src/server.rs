...
//! `crates/atlas-cli/tests/cli.rs` proves that path with eight racing
...
impl Session {
    ...
    /// per repo via `atlas_core::workspace`; this is the fallback for the
    /// operations that take a symbol reference instead.
    pub fn repo_id(&self) -> &str {
        atlas_core::ids::DEFAULT_REPO_ID
    }
}
...
This repository is coordinated by atlas: other people and other AI agents are \
...
pub fn run(cfg: McpConfig, input: impl BufRead, output: impl Write + Send + 'static) -> Result<()> {
    let store = Store::open(&atlas_core::db_path(&cfg.root))?;
