...
//!
//! The derivation deliberately **does not** consult `.golab/agent`. That file
//! is the *workspace* default for bare CLI invocations — one name, written by
//! `golab swarm join`. Two coding tools open in one repository would both read
...
pub fn os_user() -> String {
    for key in ["GOLAB_USER", "USERNAME", "USER", "LOGNAME"] {
...
///
/// Precedence: `explicit` (a `--as` flag) → `$GOLAB_AGENT` → `<tool>-<user>`.
///
...
pub fn derive_agent(tool: &str, explicit: Option<&str>) -> String {
    ...
    if let Some(name) = std::env::var("GOLAB_AGENT")
        .ok()
        .map(|s| slug(&s))
        .filter(|s| !s.is_empty())
    {
...
mod tests {
    ...
    fn with_env(agent: Option<&str>, user: &str, f: impl FnOnce()) {
        ...
        let prev_agent = std::env::var("GOLAB_AGENT").ok();
        let prev_user = std::env::var("GOLAB_USER").ok();
        match agent {
            Some(a) => std::env::set_var("GOLAB_AGENT", a),
            None => std::env::remove_var("GOLAB_AGENT"),
        }
        std::env::set_var("GOLAB_USER", user);
        f();
        match prev_agent {
            Some(a) => std::env::set_var("GOLAB_AGENT", a),
            None => std::env::remove_var("GOLAB_AGENT"),
        ...
        match prev_user {
            Some(u) => std::env::set_var("GOLAB_USER", u),
            None => std::env::remove_var("GOLAB_USER"),
        }
    }
