...
//! rather than on a server being up, which is what makes `golab lease acquire`
...
CREATE TABLE IF NOT EXISTS repos (
    id         TEXT PRIMARY KEY,
    name       TEXT NOT NULL,
    -- Relative to the directory containing .golab/, so the workspace stays
...
-- task_symbols and task_deps, it self-heals on an existing .golab/runtime.db
...
impl Store {
    /// Create `.golab/runtime.db` under `root` (idempotent).
    ...
    fn enforce_schema_version(&self) -> Result<()> {
        ...
        match stored {
            Some(v) if v != SCHEMA_VERSION => Err(anyhow!(
                "this workspace's .golab/runtime.db was written for schema v{v}, but this golab \
                 speaks v{SCHEMA_VERSION}. There are no migrations by design — delete \
                 .golab/runtime.db and re-run `golab index` to rebuild it."
            )),
            Some(_) => Ok(()),
            None => {
                self.conn.execute(
    ...
    pub fn resolve(&self, query: &str) -> Result<Symbol> {
        ...
        match candidates.len() {
            0 => Err(anyhow!(
                "no symbol matches '{query}' (run `golab scan` if the index is stale)"
            )),
...
/// here, so `golab watch`, the dashboard and audits all see the same stream.
pub fn emit(
    tx: &Transaction,
    kind: &str,
    agent: Option<&str>,
...
mod tests {
    ...
    fn a_database_from_a_different_schema_refuses_to_open() {
        ...
        assert!(
            msg.contains("delete .golab/runtime.db"),
