...
pub struct GuardSuggestion {
    ...
    /// Ready to run, e.g. `golab request lease "src/pay.ts:charge" --reason "..."`.
    pub command: String,
...
impl Store {
    ...
    pub fn guard_edit(
        ...
        report.suggestions = vec![GuardSuggestion {
            ...
            command: format!("golab lease acquire {}", quote(&handle)),
    ...
    if file_wide {
            ...
            .find(|c| c.relation == ConflictRelation::Descendant)
        {
            out.push(GuardSuggestion {
                ...
                holder: Some(c.holder.clone()),
                seconds_until_free: Some(c.seconds_until_free),
                command: format!("golab guard <path> --symbol {}", quote(&c.blocking_symbol)),
