...
pub struct Symbol {
    ...
    /// symbol until `atlas repo add` registers a second one.
    pub repo_id: String,
}
...
/// least `R1`, added automatically by `atlas init`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Repo {
    pub id: String,
    pub name: String,
    /// Relative to the directory containing `.atlas/`, so the workspace
