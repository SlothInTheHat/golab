...
//!
//! A workspace can hold more than one repo (`atlas repo add`), and `path` is
...
/// `atlas check`, repo by repo, merged into one report.
...
/// `atlas diff`, repo by repo, merged into one change list.
